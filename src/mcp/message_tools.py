...
def register_message_tools(mcp: FastMCP) -> None:
    ...
    async def create_pending_message(
        ...
        """
        Adiciona uma nova mensagem ao banco de dados com o status de 'pending' para ser processada depois.
        ...
        if type not in ["text", "img", "video", "audio"]:
            logger.warning(f"Warning: Message type '{type}' is not one of the standard types [text, img, video, audio].")
        ...
        try:
            ...
            return f"Mensagem enviada com sucesso e está pendente de processamento. ID: {message_out['id']}"
